...
const navigate = useNavigate();
  ...
  if (currentView === 'category') {
    ...
    // Handle categories with subcategories (like Laboratories)
        ...
        <section className="py-0 bg-white min-h-screen">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="text-center mb-8 pt-8">
              ...
              <div className="w-32 h-1 bg-yellow-400 rounded-full mx-auto mt-4 mb-4"></div>
              <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                Explore our state-of-the-art laboratory facilities
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-16">
              {categoryData.subcategories.map((subcategory, index) => (
                <div 
                  ...
                  className="group relative cursor-pointer overflow-hidden rounded-xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2"
                  onClick={() => navigateToSubcategory(subcategory.name)}
                >
                  <div className="aspect-w-4 aspect-h-3">
                    <img 
                      src={subcategory.images[0]?.url || '/api/placeholder/400/300'} 
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500">
                    <div className="absolute bottom-0 left-0 right-0 p-4">
                      <h3 className="text-white font-bold text-lg leading-tight mb-2">{subcategory.name}</h3>
                      <p className="text-white/90 text-sm mb-3">
                        {subcategory.images.length} image{subcategory.images.length !== 1 ? 's' : ''}
                      </p>
                      <div className="w-8 h-0.5 bg-yellow-400 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></div>
                    </div>
                  </div>
                  
                  {/* Floating number indicator */}
                  <div className="absolute top-3 right-3 bg-yellow-500 text-gray-900 text-xs font-bold px-2 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    {index + 1}
                  </div>
                </div>
              ))}
            ...
            <div className="text-center mb-8">
              <div className="inline-flex items-center gap-4 bg-gray-50 rounded-full px-6 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-700 font-medium">{categoryData.subcategories.length} Labs</span>
                </div>
            ...
            <div className="flex justify-center mt-16 pb-16">
              <button
                onClick={goBackToCarousel}
                className="flex items-center gap-2 px-8 py-4 bg-yellow-500 hover:bg-yellow-400 text-gray-900 rounded-full font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl group"
              >
                <ArrowLeft className="h-5 w-5 group-hover:-translate-x-1 transition-transform duration-300" />
                <span>Back to Gallery</span>
              </button>
    ...
    // Handle categories with direct images (like Campus, Events)
    return (
      <section className="py-0 bg-white min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Category Header */}
          ...
          {/* Innovative Masonry Grid Layout */}
          <div className="columns-1 md:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
            {currentImages.map((image, index) => (
              <div 
                key={index}
                className="break-inside-avoid relative group cursor-pointer overflow-hidden rounded-xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 mb-6"
                onClick={() => openLightbox(image, index)}
              >
                <div className="relative">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500">
                    <div className="absolute bottom-0 left-0 right-0 p-4">
                      <p className="text-white font-semibold text-sm leading-relaxed">{image.caption}</p>
                      <div className="w-8 h-0.5 bg-yellow-400 mt-2 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></div>
                    </div>
                  </div>
                </div>
          ...
          <div className="text-center mt-12 mb-8">
            <div className="inline-flex items-center gap-4 bg-gray-50 rounded-full px-6 py-3">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
                <span className="text-gray-700 font-medium">{currentImages.length} Images</span>
              </div>
              <div className="w-px h-4 bg-gray-300"></div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
            ...
            <div className="relative max-w-5xl max-h-full">
              ...
              <div className="absolute bottom-4 left-4 right-4 bg-black/70 backdrop-blur-sm text-white p-6 rounded-xl">
                ...
                <div className="flex items-center justify-center gap-4 mt-3">
                  ...
                  <div className="w-px h-4 bg-gray-500"></div>
                  <span className="text-sm text-gray-300">{selectedCategory}</span>
                </div>
              </div>
            </div>
          </div>
