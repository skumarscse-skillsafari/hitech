            ...
            {currentImages.map((image, index) => (
              ...
              >
                <div className="aspect-w-16 aspect-h-12 relative">
                  <img 
                    src={`${import.meta.env.BASE_URL}${image.url.startsWith('/') ? image.url.slice(1) : image.url}`}
                    alt={image.caption}
                    loading="lazy"
  ...
  >
    ...
    >
      ...
      <div className="flex justify-center items-center w-full max-h-[80vh] overflow-hidden">
        <img
          src={`${import.meta.env.BASE_URL}${selectedImage.url.startsWith('/') ? selectedImage.url.slice(1) : selectedImage.url}`}
          ...
          {currentCategoryData?.subcategories?.map((sub, index) => (
            ...
            >
              <div className="aspect-w-16 aspect-h-12">
                <img
                  src={
                    (sub.images?.[0]?.url || '/api/placeholder/400/300').startsWith('/') && !(sub.images?.[0]?.url || '').startsWith('/api')
                      ? `${import.meta.env.BASE_URL}${(sub.images?.[0]?.url || '/api/placeholder/400/300').slice(1)}`
                      : sub.images?.[0]?.url || '/api/placeholder/400/300'
                  }
                  alt={sub.name}
                  loading="lazy"
  ...
  <ChevronRight className="h-6 w-6" />
          ...
          >
            {categories.map((category, index) => {
              const rawPreviewUrl = category.images?.[0]?.url || category.subcategories?.[0]?.images[0]?.url || '/api/placeholder/800/450';
              const previewImage = rawPreviewUrl.startsWith('/') && !rawPreviewUrl.startsWith('/api') 
                ? `${import.meta.env.BASE_URL}${rawPreviewUrl.slice(1)}` 
                : rawPreviewUrl;
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-16">
          {categories.map((category, index) => {
            const rawPreviewUrl = category.images?.[0]?.url || category.subcategories?.[0]?.images[0]?.url || '/api/placeholder/400/300';
            const previewImage = rawPreviewUrl.startsWith('/') && !rawPreviewUrl.startsWith('/api') 
              ? `${import.meta.env.BASE_URL}${rawPreviewUrl.slice(1)}` 
              : rawPreviewUrl;
            const imageCount = category.images?.length || category.subcategories?.reduce((acc, sub) => acc + sub.images.length, 0) || 0;
