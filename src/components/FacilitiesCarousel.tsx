...
const FacilitiesCarousel: React.FC<FacilitiesCarouselProps> = ({ 
  ...
  const nextSlide = () => {
    setCurrentIndex((prevIndex) => 
      prevIndex + itemsPerPage >= facilities.length ? 0 : prevIndex + itemsPerPage
    );
  };

  const prevSlide = () => {
    setCurrentIndex((prevIndex) => 
      prevIndex === 0 ? Math.max(0, facilities.length - itemsPerPage) : Math.max(0, prevIndex - itemsPerPage)
    );
  };
