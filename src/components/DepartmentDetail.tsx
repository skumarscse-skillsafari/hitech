...
const DepartmentDetail: React.FC<DepartmentDetailProps> = ({ department }) => {
  ...
  <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
    <div className="relative h-[450px] lg:h-[500px]">
      ...
      <img 
        src={department.image.startsWith('/') ? `${import.meta.env.BASE_URL}${department.image.slice(1)}` : department.image}
        alt={department.name}
              ...
              {visibleCards.map((card) => (
                <div key={card.id} className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow duration-300">
                  <div className="h-48 overflow-hidden">
                    <img 
                      src={card.image.startsWith('/') ? `${import.meta.env.BASE_URL}${card.image.slice(1)}` : card.image}
                      alt={card.title}
                  ...
                  {eventsData.events
                    ...
                    <div key={`event-${selectedEventsYear}-${index}`} className="flex-shrink-0 w-96 bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border-2 border-yellow-400">
                      <div className="p-6">
                        ...
                        {event.image && (
                          <img 
                            src={`${import.meta.env.BASE_URL}${event.image.startsWith('/') ? event.image.slice(1) : event.image}`}
                            alt={event.eventName}
                            className="w-full h-48 object-cover rounded-lg mb-4"
                            loading="lazy"
                          />
