  ...
  return (
    ...
    >
      ...
      <div className="relative min-h-[280px] sm:h-48 overflow-hidden flex-shrink-0">
        <img
          src={
            dept.image.startsWith('http') 
              ? dept.image 
              : dept.image.startsWith('public/')
                ? `${import.meta.env.BASE_URL}${dept.image.slice(7)}`
                : dept.image.startsWith('/') 
                  ? `${import.meta.env.BASE_URL}${dept.image.slice(1)}` 
                  : dept.image
          }
          alt={dept.name}
