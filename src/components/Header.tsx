          ...
          {item.dropdown ? (
            <>
              <button className="flex justify-between items-center w-full px-4 py-3 text-sm text-gray-900 hover:bg-yellow-50 hover:text-yellow-600 transition-all duration-300 transform hover:translate-x-1">
                <span className="whitespace-normal text-left break-words pr-2">{item.name}</span>
                <ChevronDown className="ml-1 h-4 w-4 flex-shrink-0 transition-transform group-hover/sub:rotate-180" />
              </button>
              <div className="absolute top-0 right-full mr-1 w-72 bg-white border rounded-lg shadow-xl opacity-0 invisible group-hover/sub:opacity-100 group-hover/sub:visible transition-all duration-300 transform translate-y-2 group-hover/sub:translate-y-0 z-[9999]">
            ...
            <button
              ...
              className="block w-full text-left px-4 py-3 text-sm text-gray-900 hover:bg-yellow-50 hover:text-yellow-600 transition-all duration-300 transform hover:translate-x-1 whitespace-normal break-words"
            >
              {item.name}
            </button>
          ) : (
            <Link
              to={item.href}
              className="block px-4 py-3 text-sm text-gray-900 hover:bg-yellow-50 hover:text-yellow-600 transition-all duration-300 transform hover:translate-x-1 whitespace-normal break-words"
        ...
        <div className="flex items-center justify-between min-h-[4rem] gap-2 lg:gap-8">
          ...
          >
            <div className="bg-white p-1 rounded-lg shadow-md transition-all duration-300 group-hover/logo:shadow-lg group-hover/logo:ring-2 group-hover/logo:ring-yellow-200 flex-shrink-0">
              <img src={`${import.meta.env.BASE_URL}Logo.jpg`} alt="Logo" className="h-10 w-auto sm:h-12 object-contain" />
            </div>
          ...
          <nav className="hidden lg:flex items-center gap-1 flex-shrink-0">
              ...
              <div key={item.name} className="relative group">
                {item.dropdown ? (
                  <div className="relative" onMouseLeave={() => setActiveDropdown(null)}>
                    ...
                    <div
                      className="absolute top-full left-0 mt-1 w-72 bg-white rounded-lg shadow-xl border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[9999]"
            ...
            <div className="relative ml-4">
              ...
              {activeDropdown === 'hamburger' && (
                <div className="absolute right-0 mt-2 w-56 bg-white border rounded-lg shadow-xl z-[9999]">
                  <Link
                    to="/online-fees"
