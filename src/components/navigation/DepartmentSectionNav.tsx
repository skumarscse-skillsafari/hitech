...
const DepartmentSectionNav: React.FC<DepartmentSectionNavProps> = ({ sections }) => {
  ...
  return (
    <>
      ...
      <nav
        className={`
          fixed lg:sticky top-0 lg:top-32 left-0 h-screen
          ...
          lg:h-[calc(100vh-9rem)] lg:max-h-[calc(100vh-9rem)]
          ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
        `}
      >
        <div 
          className="h-full overflow-y-auto p-6 lg:p-3 space-y-1"
          style={{
            scrollbarWidth: 'thin',
              ...
              return (
                <li key={section.id} className="border-b border-black last:border-b-0">
                  <button
                    ...
                    className={`
                      group relative w-full text-left flex items-center px-4 py-3 text-sm font-medium transition hover:bg-yellow-600 border-r border-yellow-600 last:border-r-0
